...
if not os.path.exists(os.path.join(wd, xp_name)):
	os.mkdir(os.path.join(wd, xp_name))
...
emb_dim, n_layers, heads = 512, 12, 12
embedding = nn.Embedding(voc.num_words, emb_dim)
model = Transformer(vocab_size=voc.num_words, emb_dim=emb_dim, embedding = embedding, n_layers=n_layers, heads=heads, dropout=0.01)
model.to(device)
lr = 0.0001
lr_str = '0001'
...
tb = SummaryWriter(comment='/' + xp_name + 'lr' + str(lr) + '_bs' + str(bs) + '_ed' + str(emb_dim)
